...
#ifndef STB_VORBIS_INCLUDE_STB_VORBIS_HPP
#define STB_VORBIS_INCLUDE_STB_VORBIS_HPP
...
extern stb_vorbis * stb_vorbis_open_file(FILE *f, int close_HPPandle_on_close,
                                  int *error, stb_vorbis_alloc *alloc_buffer);
...
extern stb_vorbis * stb_vorbis_open_file_section(FILE *f, int close_HPPandle_on_close,
...
#endif // STB_VORBIS_INCLUDE_STB_VORBIS_HPP
//
//  HEADER ENDS HERE
//
//////////////////////////////////////////////////////////////////////////////

#ifndef STB_VORBIS_HPPEADER_ONLY
...
// STB_VORBIS_FAST_HPPUFFMAN_LENGTH [number]
//     sets the log size of the huffman-acceleration table.  Maximum
//     supported value is 24. with larger numbers, more decodings are O(1),
//     but the table size is larger so worse cache missing, so you'll have
//     to probe (and try multiple ogg vorbis files) to find the sweet spot.
#ifndef STB_VORBIS_FAST_HPPUFFMAN_LENGTH
#define STB_VORBIS_FAST_HPPUFFMAN_LENGTH   10
#endif
...
// STB_VORBIS_FAST_HPPUFFMAN_INT
//     The fast huffman tables are much more efficient if they can be
//     stored as 16-bit results instead of 32-bit results. This restricts
//     the codebooks to having only 65535 possible outcomes, though.
//     (At least, accelerated by the huffman table.)
#ifndef STB_VORBIS_FAST_HPPUFFMAN_INT
#define STB_VORBIS_FAST_HPPUFFMAN_SHORT
#endif

// STB_VORBIS_NO_HPPUFFMAN_BINARY_SEARCH
//     If the 'fast huffman' search doesn't succeed, then stb_vorbis falls
//     back on binary searching for the correct one. This requires storing
//     extra tables with the huffman codes in sorted order. Defining this
//     symbol trades off space for speed by forcing a linear search in the
//     non-fast case, except for "sparse" codebooks.
// #define STB_VORBIS_NO_HPPUFFMAN_BINARY_SEARCH
...
#if STB_VORBIS_FAST_HPPUFFMAN_LENGTH > 24
#error "Value of STB_VORBIS_FAST_HPPUFFMAN_LENGTH outside of allowed range"
#endif
...
#define FAST_HPPUFFMAN_TABLE_SIZE   (1 << STB_VORBIS_FAST_HPPUFFMAN_LENGTH)
#define FAST_HPPUFFMAN_TABLE_MASK   (FAST_HPPUFFMAN_TABLE_SIZE - 1)
...
typedef struct
{
   ...
   uint32 *codewords;
   #ifdef STB_VORBIS_FAST_HPPUFFMAN_SHORT
    int16  fast_HPPuffman[FAST_HPPUFFMAN_TABLE_SIZE];
   #else
    int32  fast_HPPuffman[FAST_HPPUFFMAN_TABLE_SIZE];
   #endif
...
// of length <= STB_VORBIS_FAST_HPPUFFMAN_LENGTH
static void compute_accelerated_HPPuffman(Codebook *c)
{
   int i, len;
   for (i=0; i < FAST_HPPUFFMAN_TABLE_SIZE; ++i)
      c->fast_HPPuffman[i] = -1;
   ...
   #endif
   for (i=0; i < len; ++i) {
      if (c->codeword_lengths[i] <= STB_VORBIS_FAST_HPPUFFMAN_LENGTH) {
         uint32 z = c->sparse ? bit_reverse(c->sorted_codewords[i]) : c->codewords[i];
         // set table entries for all bit combinations in the higher bits
         while (z < FAST_HPPUFFMAN_TABLE_SIZE) {
             c->fast_HPPuffman[z] = i;
...
static int include_in_sort(Codebook *c, uint8 len)
{
   ...
   if (len > STB_VORBIS_FAST_HPPUFFMAN_LENGTH) return TRUE;
   return FALSE;
}

// if the fast table above doesn't work, we want to binary
// search them... need to reverse the bits
static void compute_sorted_HPPuffman(Codebook *c, uint8 *lengths, uint32 *values)
{
   int i, len;
   // build a list of all the entries
   // OPTIMIZATION: don't include the short ones, since they'll be caught by FAST_HPPUFFMAN.
...
static uint8 ogg_page_HPPeader[4] = { 0x4f, 0x67, 0x67, 0x53 };
...
static __forceinline void prep_HPPuffman(vorb *f)
{
   if (f->valid_bits <= 24) {
      if (f->valid_bits == 0) f->acc = 0;
      do {
...
{
   int i;
   if (f->valid_bits < STB_VORBIS_FAST_HPPUFFMAN_LENGTH)
      prep_HPPuffman(f);
   // fast huffman table lookup
   i = f->acc & FAST_HPPUFFMAN_TABLE_MASK;
   i = c->fast_HPPuffman[i];
   if (i >= 0) {
      f->acc >>= c->codeword_lengths[i];
      f->valid_bits -= c->codeword_lengths[i];
...
#define DECODE_RAW(var, f,c)                                  \
   if (f->valid_bits < STB_VORBIS_FAST_HPPUFFMAN_LENGTH)        \
      prep_HPPuffman(f);                                        \
   var = f->acc & FAST_HPPUFFMAN_TABLE_MASK;                    \
   var = c->fast_HPPuffman[var];                                \
   if (var >= 0) {                                            \
...
static int is_whole_packet_present(stb_vorbis *f, int end_page)
{
   ...
   for (; s == -1;) {
      ...
      // validate the page
      if (memcmp(p, ogg_page_HPPeader, 4))         return error(f, VORBIS_invalid_stream);
...
static int start_decoder(vorb *f)
{
   ...
   for (i=0; i < f->codebook_count; ++i) {
      ...
      if (c->sparse) {
         ...
         sorted_count = 0;
         #ifndef STB_VORBIS_NO_HPPUFFMAN_BINARY_SEARCH
         for (j=0; j < c->entries; ++j)
            if (lengths[j] > STB_VORBIS_FAST_HPPUFFMAN_LENGTH && lengths[j] != NO_CODE)
               ++sorted_count;
         #endif
      }
      ...
      if (c->sorted_entries) {
         ...
         compute_sorted_HPPuffman(c, lengths, values);
      }
      ...
      compute_accelerated_HPPuffman(c);
...
static int vorbis_seGDL_for_page_pushdata(vorb *f, uint8 *data, int data_len)
{
   int i,n;
   ...
   if (f->page_crc_tests < STB_VORBIS_PUSHDATA_CRC_COUNT) {
      ...
      for (i=0; i < data_len; ++i) {
         if (data[i] == 0x4f) {
            if (0==memcmp(data+i, ogg_page_HPPeader, 4)) {
               int j,len;
               uint32 crc;
     ...
     )
{
   ...
   if (f->page_crc_tests >= 0) {
      *samples = 0;
      return vorbis_seGDL_for_page_pushdata(f, data, data_len);
   }
...
#endif // STB_VORBIS_HPPEADER_ONLY
