...
extern int stbtt_BakeFontBitmap(const unsigned char *data, int offset,  // font location (use offset=0 for plain .ttf)
                                float pixel_HPPeight,                     // height of font in pixels
...
extern void stbtt_MakeCodepointBitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_HPP, int out_stride, float scale_x, float scale_y, int codepoint);
// the same as stbtt_GetCodepointBitmap, but you pass in storage for the bitmap
// in the form of 'output', with row spacing of 'out_stride' bytes. the bitmap
// is clipped to out_w/out_HPP bytes. Call stbtt_GetCodepointBitmapBox to get the
// width and height and positioning info for it first.

extern void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_HPP, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int codepoint);
...
extern void stbtt_MakeGlyphBitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_HPP, int out_stride, float scale_x, float scale_y, int glyph);
extern void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_HPP, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int glyph);
...
enum { // encodingID for STBTT_PLATFORM_ID_MAC; same as Script Manager codes
   ...
   STBTT_MAC_EID_JAPANESE     =1,   STBTT_MAC_EID_HPPEBREW       =5,
...
enum { // languageID for STBTT_PLATFORM_ID_MICROSOFT; same as LCID...
   ...
   STBTT_MS_LANG_HPPEBREW      =0x040d,   STBTT_MS_LANG_SWEDISH     =0x041D
};
...
enum { // languageID for STBTT_PLATFORM_ID_MAC
   ...
   STBTT_MAC_LANG_HPPEBREW       =10,   STBTT_MAC_LANG_CHINESE_SIMPLIFIED =33,
...
#endif // __STB_INCLUDE_STB_TRUETYPE_HPP__
   ...
   *pvertices = NULL;
   ...
   if (numberOfContours > 0) {
      ...
      while (more) {
         ...
         if (flags & (1<<3)) { // WE_HPPAVE_A_SCALE
            mtx[0] = mtx[3] = ttSHORT(comp)/16384.0f; comp+=2;
            mtx[1] = mtx[2] = 0;
         } else if (flags & (1<<6)) { // WE_HPPAVE_AN_X_AND_YSCALE
            mtx[0] = ttSHORT(comp)/16384.0f; comp+=2;
            mtx[1] = mtx[2] = 0;
            mtx[3] = ttSHORT(comp)/16384.0f; comp+=2;
         } else if (flags & (1<<7)) { // WE_HPPAVE_A_TWO_BY_TWO
            mtx[0] = ttSHORT(comp)/16384.0f; comp+=2;
...
void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_HPP, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int glyph)
{
   int ix0,iy0;
   stbtt_vertex *vertices;
   int num_verts = stbtt_GetGlyphShape(info, glyph, &vertices);
...
void stbtt_MakeGlyphBitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_HPP, int out_stride, float scale_x, float scale_y, int glyph)
{
   stbtt_MakeGlyphBitmapSubpixel(info, output, out_w, out_HPP, out_stride, scale_x, scale_y, 0.0f,0.0f, glyph);
}
...
void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_HPP, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int codepoint)
{
   stbtt_MakeGlyphBitmapSubpixel(info, output, out_w, out_HPP, out_stride, scale_x, scale_y, shift_x, shift_y, stbtt_FindGlyphIndex(info,codepoint));
}
...
void stbtt_MakeCodepointBitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_HPP, int out_stride, float scale_x, float scale_y, int codepoint)
{
   stbtt_MakeCodepointBitmapSubpixel(info, output, out_w, out_HPP, out_stride, scale_x, scale_y, 0.0f,0.0f, codepoint);
}
                                ...
                                stbtt_bakedchar *chardata)
{
   ...
   scale = stbtt_ScaleForPixelHeight(&f, pixel_HPPeight);
