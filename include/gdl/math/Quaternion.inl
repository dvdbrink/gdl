...
inline math::Vector3f operator*(const Quaternion& q, const math::Vector3f& v)
{
	math::Vector3f uv, uuv;
	math::Vector3f qvec{q.x, q.y, q.z};
	uv = cross(qvec, v);
	uuv = cross(qvec, uv);
	uv *= (2.0f * q.w);
	uuv *= 2.0f;
...
inline math::Vector3f operator*(const math::Vector3f& v, const Quaternion& q)
{
	return (Quaternion(-q.x, -q.y, -q.z, q.w) / dot(q, q)) * v;
}

inline Quaternion rotate(const Quaternion& in, const float angle, const math::Vector3f& unit)
{
